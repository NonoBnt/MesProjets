//NOM prénom étudiant 1 :Nina Metrouh
//NOM prénom étudiant 2 :Arnaud Bonnet
...
*/
        ...
        Tunite *tourRoi =creeTourRoi(4,1);
        Tunite *first=creeGargouille(tabParcours[0][0],tabParcours[0][1]);

        roi=ajoutEnTete(roi, *tourRoi);
        PositionnePlayerOnPlateau(roi, jeu);

        horde=ajoutEnTete(horde, *first);
        PositionnePlayerOnPlateau(horde, jeu);

        bool creationtour, creationhorde;
        ...
        while ( tourRoiDetruite(roi)!=true ){   //VOUS DEVEZ GERER (DETECTER) LA FIN DU JEU -> tourRoiDetruite
                ...
                if(horde!=NULL){
                    printf("%s\n", "roi attaque");
                    phase1combat(pWinSurf, roi, horde, jeu);
                    printf("%s\n", "roi a attaque");
                    printf("%s\n", "horde attaque");
                    phase2combat(pWinSurf, roi, horde, jeu);
                    printf("pv roi : %d\n", roi->pdata->pointsDeVie);
                    printf("%s\n", "horde a attaque");
                    horde=supprimerUnite(horde, horde->pdata, jeu);
                    moveUnite(horde, jeu, tabParcours);
                }

                printf("%s\n", "mouvement");

                creationhorde=creationpossible(50,80);

                if(creationhorde==true){
                    AjouterUnite(&horde, randomunitehorde(tabParcours[0][0],tabParcours[0][1]));
                    PositionnePlayerOnPlateau(horde, jeu);
                    printf("creation horde\n");
                } else printf("pas de horde creer\n");

                creationtour=creationpossible(15,50);

                printf("creation possible de tour : %d \n", creationtour);
                if(creationtour==true){
                    if(getNbreCell(roi)<=10){
                        creerrandomtour(roi, jeu, tabParcours);
                        PositionnePlayerOnPlateau(roi, jeu);
                        printf("creation tour\n\n");
                    }
                } else printf("pas de tour creer\n\n");

                //gerer enlever le droit d'attaquer
...
/*
                ...
                */
                ...
                maj_fenetre(pWindow);
                SDL_Delay(600);  //valeur du délai à modifier éventuellement
                ...
                if ( pKeyStates[SDL_SCANCODE_V] ){
                        ...
                        readseq("partieseq.cls", roi, horde);
                ...
                if ( pKeyStates[SDL_SCANCODE_C] ){
                        ...
                        // APPELEZ ICI VOTRE FONCTION DE SAUVEGARDE/RESTAURATION DEMANDEE
                        //message("Sauvegarde","Placer ici votre fonction de restauration/sauvegarde");
                        readbin("partiebin.clb", roi, horde);
                        //Ne pas modifiez les 4 lignes ci-dessous
                        efface_fenetre(pWinSurf);
                        prepareAllSpriteDuJeu(jeu,tabParcours,LARGEURJEU,HAUTEURJEU,TabSprite,pWinSurf);
                        maj_fenetre(pWindow);
                ...
                if ( pKeyStates[SDL_SCANCODE_D] ){
                        ...
                        // APPELEZ ICI VOTRE FONCTION DE SAUVEGARDE/RESTAURATION DEMANDEE
                        //message("Sauvegarde","Placer ici votre fonction de restauration/sauvegarde");
                        sauvegardeseq("partieseq.cls", roi, horde);
                        //Ne pas modifiez les 4 lignes ci-dessous
                        efface_fenetre(pWinSurf);
                        prepareAllSpriteDuJeu(jeu,tabParcours,LARGEURJEU,HAUTEURJEU,TabSprite,pWinSurf);
                        maj_fenetre(pWindow);
                ...
                if ( pKeyStates[SDL_SCANCODE_S] ){
                        ...
                        // APPELEZ ICI VOTRE FONCTION DE SAUVEGARDE/RESTAURATION DEMANDEE
                        //message("Sauvegarde","Placer ici votre fonction de restauration/sauvegarde");
                        sauvegardebin("partiebin.clb", roi, horde);
                ...
                if ( pKeyStates[SDL_SCANCODE_ESCAPE] ){
                        roi->pdata->pointsDeVie=-10;  //sortie de la boucle
                }
